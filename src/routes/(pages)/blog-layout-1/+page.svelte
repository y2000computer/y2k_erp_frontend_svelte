		...
		{#each _.take(fakerData, 9) as faker, fakerKey}
			<div class="col-span-12 intro-y md:col-span-6 xl:col-span-4 box">
				...
				>
					...
					<DropdownMenu.Root>
						<DropdownMenu.Trigger asChild let:builder>
							<a href="/" class="w-5 h-5 ml-3 text-slate-500" use:builder.action {...builder}>
								<Lucide icon="EllipsisVertical" class="w-5 h-5" />
							</a>
						</DropdownMenu.Trigger>
						<DropdownMenu.Content class="w-40">
							<DropdownMenu.Item>
								<Lucide icon="Pencil" class="w-4 h-4 mr-2" /> Edit Post
							</DropdownMenu.Item>
							<DropdownMenu.Item>
				...
				<div class="px-5 pt-3 pb-5 border-t border-slate-200/60 dark:border-darkmode-400">
					...
					<div class="flex items-center w-full mt-3">
						...
						<div class="relative flex-1 text-slate-600">
							...
							/>
							<Lucide icon="Smile" class="absolute inset-y-0 right-0 w-4 h-4 my-auto mr-3" />
						</div>
					</div>
				</div>
			</div>
